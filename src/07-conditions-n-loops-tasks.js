...
function doRectanglesOverlap(rect1, rect2) {
  if (rect1.left >= rect1.left + rect1.width) return false;
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  const res = [Math.min(a, b), Math.max(a, b)];
  const open = isStartIncluded ? '[' : '(';
  const close = isEndIncluded ? ']' : ')';
  return `${open}${res[0]}, ${res[1]}${close}`;
}
...
function getDigitalRoot(n) {
  const sum = n.toString().split('').reduce((acc, val) => acc + Number(val), 0);
